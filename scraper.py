...
output_filepath = "./Mountain Home addresses.geojson"  # Filepath to save the merged GeoJSON
...
def fetch_page(result_offset):
    ...
    """
    url = f"https://services.arcgis.com/91hXl6NfvLGEi8x5/arcgis/rest/services/Elmore_Roads_Layer/FeatureServer/0/query"
    params = {
        "f": "geojson",
        "where": "1=1",
        "returnGeometry": "true",
        "outFields": "Add_Number,AddNum_Suf,Post_Code,UNIT,StN_PreDir,StN_PreTyp,StreetName,StN_PosTyp,StN_PosDir",
        "outSR": 3857, # WGS 84
    ...
    if response.status_code == 200:
        ...
        print(f"Scraped {len(data['features'])} (total: {result_offset})")
...
while (features := fetch_page(result_offset)) and result_offset < min_features and len(features) != 0:
    ...
    for feature in features:
        ...
        properties["addr:housenumber"] = ' '.join([str(properties["Add_Number"]),properties["AddNum_Suf"]])
        del properties["Add_Number"]
        properties["addr:postcode"] = properties["Post_Code"]
        del properties["Post_Code"]
        properties["addr:unit"] = properties["UNIT"]
        del properties["UNIT"]
        del properties["AddNum_Suf"]
        
        properties["addr:street"] = ' '.join([properties["StN_PreDir"],properties["StN_PreTyp"],properties["StreetName"],properties["StN_PosTyp"],properties["StN_PosDir"]])
        del properties["StN_PreDir"]
        del properties["StN_PreTyp"]
        del properties["StreetName"]
        del properties["StN_PosTyp"]
        del properties["StN_PosDir"]
        ...
        properties["addr:street"] = re.sub(" {2,}"," ",properties["addr:street"])
        properties["addr:street"] = re.sub(" $","",properties["addr:street"])
        properties["addr:street"] = re.sub("^ ","",properties["addr:street"])
